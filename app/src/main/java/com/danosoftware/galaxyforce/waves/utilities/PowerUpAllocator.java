 ...
 * <p>
 * After construction, each alien must call the allocate() method and will either receive a power-up
 * or a null.
 * <p>
 * Allocator guarantees that all power-ups will be allocated by the time the last alien has
 * requested an allocation.
 * <p>
 * Since each alien can only be allocated 1 or 0 power-ups, if an allocator is constructed with more
 * power-ups than aliens, then an exception will be thrown.
 */
  ...
  PowerUpAllocator(
    ...
    if (filteredPowerUps.size() > aliensToAllocate) {
      throw new GalaxyForceException(
          "Attempted to allocate " + powerUps.size() + " power-ups to only " + aliensToAllocate
              + " aliens."
      );
    }
  }
