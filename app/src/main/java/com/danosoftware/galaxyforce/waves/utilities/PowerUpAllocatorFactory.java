 ...
 * <p>
 * The power-up factory also tracks the number of life power-ups allocated. This ensures lives
 * allocated can not exceed the maximum number of lives possible.
 */
public class PowerUpAllocatorFactory {
  ...
  /**
   * Reset the power-up allocation factory for a new wave. All sub-waves for a wave are created at
   * the same time so the factory needs to know at the start of each wave how many life power-ups
   * can be allocated across all sub-waves (based on the current lives remaining).
   */
  void newWave() {
  ...
  /**
   * Create an allocator to provide power-ups. Allocator will only supply power-ups up to the
   * maximum lives possible.
   */
